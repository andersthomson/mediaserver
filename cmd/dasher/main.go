...
func encodeAction(streamno int, m scrape.Msp, dir string) float64 {
	inputFName := InputFName(streamno, m)
	outputFName := DasherReadyFilename(streamno, m) + "-fragmented.mp4"

	gopMs, gopFrames := gop(inputFName, m.Dash.Streams[streamno].Source, dir)
	var args []string
	...
	case "x264":
		...
		args = []string{
			...
			"-preset:v", "veryfast",
			"-tune:v", tune,
			"-x264-params:v", "keyint=" + strconv.FormatFloat(gopFrames, 'f', 0, 64) + ":min-keyint=" + strconv.FormatFloat(gopFrames, 'f', 0, 64) + ":scenecut=0:open-gop=0:vbv-maxrate=" + strconv.Itoa(bitrate) + ":vbv-bufsize=" + strconv.Itoa(bufsize) + ":crf-max=" + strconv.Itoa(crfMax),
	...
	case "x265":
		...
		args = append(args,
			"-tag:v", "hvc1",
			"-x265-params:v", "keyint="+strconv.FormatFloat(gopFrames, 'f', 0, 64)+":min-keyint="+strconv.FormatFloat(gopFrames, 'f', 0, 64)+":scenecut=0:open-gop=0:vbv-maxrate="+strconv.Itoa(bitrate)+":vbv-bufsize="+strconv.Itoa(bufsize),
			"-movflags", "frag_keyframe+empty_moov+default_base_moof",
			outputFName,
		)
	}
	...
	cmd = exec.Command("/usr/bin/MP4Box", "-dash", strconv.FormatFloat(gopMs, 'f', 0, 64), "-rap", "-profile", "onDemand", "-segment-name", outputFName+"-postDash.mp4", "-out", "manifest.mpd", outputFName)
	cmd.Dir = DashDir(m)
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	cmd.Stdin = nil
...
func makeDashWorkFlow(dir string, mspFile string) error {
	...
	for streamno, stream := range m.Dash.Streams {
		switch stream.Codec {
		case "x264":
			gopMs = encodeAction(streamno, m, dir)
		case "x265":
			gopMs = encodeAction(streamno, m, dir)
		case "reference":
